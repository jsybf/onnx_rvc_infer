...
class ContentVec:
    ...
    def __init__(self, vec_path: pathlib.Path, device=None, log_level: int = 2):
        self.sess_opt = ort_dml.SessionOptions()
        self.sess_opt.graph_optimization_level = ort_dml.GraphOptimizationLevel.ORT_ENABLE_EXTENDED
        self.sess_opt.log_severity_level = log_level
        self.sess_opt.enable_profiling = True
        ...
        logger.info(f"loading hubert model path:{vec_path}")
        self.model = ort_dml.InferenceSession(vec_path, providers=providers, sess_options=self.sess_opt)
...
def get_f0_predictor(f0_predictor, hop_length, sampling_rate, **kargs):
    if f0_predictor == "pm":
        from src.modules.F0Predictor.PMF0Predictor import PMF0Predictor

        logger.info(f"loading f0_predicator: pm")
        f0_predictor_object = PMF0Predictor(
            sampling_rate=sampling_rate
        )
    elif f0_predictor == "harvest":
        from src.modules.F0Predictor.HarvestF0Predictor import (
            HarvestF0Predictor,
        )

        logger.info(f"loading f0_predicator: harvest")
        f0_predictor_object = HarvestF0Predictor(
            hop_length=hop_length, sampling_rate=sampling_rate
        )
    elif f0_predictor == "dio":
        from src.modules.F0Predictor.DioF0Predictor import DioF0Predictor

        logger.info(f"loading f0_predicator: dio")
...
class OnnxRVC:
    def __init__(
        ...
        self.vec_model = ContentVec(vec_path, device, log_level)
        ...
        logger.info(f"loading net_g model path:{model_path}")

        self.model = ort_dml.InferenceSession(model_path, providers=providers, sess_options=self.sess_opt)
    ...
    def segment_audio(self, wav, sr):
        ...
        while start_idx < len(wav):
            ...
            rms_lookback = \
                librosa.feature.rms(y=segment_lookback, frame_length=int(0.02 * sr), hop_length=int(0.02 * sr))[0]
    ...
    def inference(
        ...
        # Loading the sample-data audio
        # wav, sr = librosa.load(raw_path, sr=self.sampling_rate)
        # wav, sr = soundfile.read(raw_path, samplerate=self.sampling_rate, format= "WAV")
        wav, sr = soundfile.read(raw_path, dtype='float32')

        if sr != self.sampling_rate:
            data = librosa.resample(wav, orig_sr=sr, target_sr=self.sampling_rate)
            sr = self.sampling_rate

        org_length = len(wav)
        if org_length / sr > 50.0:
            logger.error(" Above 50sec audio detected. Splitting initiated ")
        ...
        for idx, (seg, seg_resampled) in enumerate(zip(segments, segments_resampled)):
            logger.info(f"Processing segment {idx + 1} of {len(segments)}")
            logger.info(f"Segment duration: {len(seg) / self.sampling_rate} seconds")
        ...
        logger.info(f"Final concatenated output duration: {len(final_output) / self.sampling_rate} seconds")
